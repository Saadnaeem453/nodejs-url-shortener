...
export default function UrlShortener({ handleShortUrl }) {
  ...
  const handleShorten = async () => {
    try {
      const result = await handleShortUrl(url);
      if (result && result.id) {
        setShortUrl(result.id);
        setError("");
      } else {
  ...
  return (
    <div className="mt-6 flex flex-col items-center w-full max-w-lg">
      ...
      <div class="relative w-full min-w-[200px] h-10">
        ...
        >
          <FaLink className="mr-2" /> Shorten URL
        </button>
      </div>
      ...
      {shortUrl && (
        <div className="mt-4 text-lg text-blue-500">
          Short URL:{" "}
          <a href={`http://localhost:8000/url/${shortUrl}`}>
